...
try:
    ...
    if (
        QMessageBox.question(
            None,
            "Eclair dependencies not installed",
        ...
        QMessageBox.information(
            None,
            "Information",
            "Packages not installed. Eclair will not function unless cetk is installed manually, see https://github.com/foclair/cetk for installation instructions.",
        )
...
class EclairDock(QDockWidget):
    def __init__(self, parent):
        ...
        btn_action_existing_database = QPushButton(
            "Connect to existing database", self.tab_db
        )
        ...
        btn_action_new_database = QPushButton(
            "Create and connect to new database", self.tab_db
        )
        ...
        btn_action_validate_sources = QPushButton(
            "Validate data (before importing)", self.tab_import
        )
        ...
        label = QLabel(
            "Edit data using QGIS functionality, or delete selected sources "
            "using button below.",
            self.tab_edit,
        )
        layout_edit.addWidget(label)
        btn_action_delete_sources = QPushButton(
            "Delete selected sources", self.tab_edit
        )
        ...
        label = QLabel(
            "Create a Excel file with aggregated emissions per sector.",
            self.tab_calculate,
        )
        ...
        label = QLabel(
            "Create NetCDF files with rasterized emissions for all substances.",
            self.tab_calculate,
        )
        layout_calculate.addWidget(label)
        btn_action_raster = QPushButton(
            " Calculate rasters of emissions", self.tab_calculate
        )
        ...
        label = QLabel(
            "Load source geometries without emissions (dynamic)", self.tab_visualize
        )
        ...
        label = QLabel(
            "Load source geometries with emissions (static)\n"
            "Layers have to be re-loaded each time the inventory is updated.",
            self.tab_visualize,
        )
        layout_visualize.addWidget(label)
        ...
        btn_action_visualize_join_point.clicked.connect(
            self.load_joined_pointsource_canvas
        )
        ...
        btn_action_visualize_join_area.clicked.connect(
            self.load_joined_areasource_canvas
        )
        ...
        btn_action_visualize_join_road.clicked.connect(
            self.load_joined_roadsource_canvas
        )
        ...
        btn_action_visualize_join_grid.clicked.connect(
            self.load_joined_gridsource_canvas
        )
    ...
    def update_db_label(self):
        ...
        if db_path == "Database not set yet.":
            db_path, has_db = project.readEntry(
                "eclair", "database", "Database not set yet."
            )
    ...
    def load_existing_database_dialog(self):
        db_path, _ = QFileDialog.getOpenFileName(
            self.tab_db, "Open SQLite database", "", "Database (*.gpkg)"
        )
        if db_path == "":
            # user cancelled
            ...
            self.update_db_label()
            message_box(
                "Load database", f"Database succesfully chosen database {db_path}."
            )

    def create_new_database_dialog(self):
        db_path, _ = QFileDialog.getSaveFileName(
            None, "Create new SQLite database", "", "Database (*.gpkg)"
        )
        if db_path == "":
            # user cancelled
            ...
            try:
                ...
                message_box(
                    "Created database",
                    f"Successfully created database {db_path} with coordinate system {epsg}",
                )
    ...
    def import_sources_dialog(self):
        file_path, _ = QFileDialog.getOpenFileName(
            None,
            "Open spreadsheet with point- and/or areasource data file",
            "",
        ...
        if file_path:  # if file_path not empty string (user did not click cancel)
            ...
            if result == QDialog.Accepted:
                ...
                if self.dry_run:
                    message_box(
                        "Validation progress",
                        "Dialog closed, validation cancelled. Restart data validation and click Validate sheets button instead if validation is desired.",
                    )
                    return
                else:
                    message_box(
                        "Import progress",
                        "Dialog closed, data import cancelled. Restart data import and click Import sheets button instead if data import is desired.",
                    )
                    return
                sheets = SHEET_NAMES
    ...
    def delete_sources(self):
        ...
        if (
            layer is None
            or not isinstance(layer, QgsMapLayer)
            or layer.type() != QgsMapLayer.VectorLayer
        ):
            message_box(
                "Delete sources",
                "No vector layer selected yet. "
        ...
        if layer.name() not in layer_names:
            message_box(
                "Delete sources",
                "The selected vector layer is not a dynamic "
        ...
        if selected_features:
            ...
            if response == QMessageBox.Yes:
                ...
                if "Successfully" in stdout.decode("utf-8"):
                    ...
                    message_box(
                        "Delete sources",
                        f"Something went wrong, error info {stderr.decode('utf-8')}",
                    )
        else:
            message_box(
                "Delete sources",
    ...
    def load_joined_gridsource_canvas(self):
        try:
            ...
            if self.db_path is None:
                message_box(
                    "Load GridSource error",
                    "Database not set yet, connect to "
            ...
            if len(result) > 0:
                ...
                result = cursor.fetchall()
                if len(result) > 0:
                    gss_source_id, gridsourcesubstance_rasters = zip(
                ...
                result = cursor.fetchall()
                if len(result) > 0:
                    gsa_source_id, gridsourceactivity_rasters = zip(
                        *[(x[0], x[1]) for x in result]
                    )
                else:
                ...
                for id, name in zip(source_id, source_name):
                    ...
                    for raster in unique_rasters:
                        raster_layer = QgsRasterLayer(
                            f"GPKG:{self.db_path}:raster_{raster}", raster
                        )
                        raster_extent = (
                        ...
                        cellsize.append(
                            (raster_extent[2] - raster_extent[0])
                    ...
                    if len(srid) != 1:
                        message_box(
                            "Load layers error",
                            f"Cannot load grids with undefined or multiple srid for gridsource {name}",
                        )
                    else:
                        srid = srid.pop()
                    ...
                    self.outputpath = os.path.join(
                        gettempdir(), dbname + "-" + name + "-" + timestamp
                    )
    ...
    def load_join(self):
        ...
        if self.source_type == "point":
            ...
            message_box(
                "Warning", f"Cannot load layer, sourcetype {source_type} unknown."
            )
        ...
        if self.source_type in ["area", "point"]:
            ...
            ac1_join = processing.run(
                "qgis:joinattributestable", parameters_codeset_join
            )["OUTPUT"]
...
class CheckboxDialog(QDialog):
    ...
    def import_sheets_dialog(self):
        ...
        self.sheet_names = [
            label for label in self.box_labels if self.checkboxes[label].isChecked()
        ]
        self.accept()
...
class RasterizeDialog(QDialog):
    ...
    def initUI(self):
        ...
        extent_label = QLabel(
            "Enter x and y coordinates for lower left (x1, y1) and upper right (x2, y2) corners of output extent:"
        )
        ...
        date_label = QLabel(
            "If one raster per hour is desired, enter begin and end date for rasters (optional)."
        )
    ...
    def run_rasterizer(self):
        ...
        if self.raster_srid < 1024 or self.raster_srid > 32767:
            message_box(
                "Rasterize error",
                "EPSG codes defining coordinate systems should be between 1024 and 32767.",
            )
            return
        self.extent = [
            float(self.extent_input[label].text()) for label in self.extent_labels
        ]
        if self.extent[2] <= self.extent[0] or self.extent[3] <= self.extent[1]:
            message_box(
                "Rasterize error",
                "Unvalid extent, x2 should be larger than x1 and y2 larger than y1.",
            )
            return
        resolution = [
            float(self.resolution_input[label].text())
            for label in self.resolution_labels
        ]
        if resolution[0] <= 0:
            message_box(
                "Rasterize error",
                "Unvalid resolution, should be a number larger than 0.",
            )
            return
        ...
        if self.date[0] != "":
            try:
                ...
                message_box(
                    "Rasterize error",
                    "Incorrect begin date. For example, 1 february 2022 has the format: 2022-02-01",
                )
                return
            if self.date[1] != "":
                try:
                    ...
                    message_box(
                        "Rasterize error",
                        "Incorrect end date. For example, 1 february 2022 has the format: 2022-02-01",
                    )
                    return
                if begin > end:
                    message_box(
                        "Rasterize error", "End date has to be after begin date."
                    )
                    return
            else:
                message_box(
                    "Rasterize error",
                    "If begin date is specified, end date has to be specified too.",
                )
                return
        elif self.date[1] != "":
            message_box(
                "Rasterize error",
                "If end date is specified, begin date has to be specified too.",
            )
            return
...
class ChooseCodesetDialog(QDialog):
    ...
    def initUI(self):
        ...
        for codeset_i in self.codesets:
            ...
            btn_action.clicked.connect(
                lambda checked, cs=codeset_i: self.run_aggregation(cs)
            )
        self.setLayout(layout)
    ...
    def run_aggregation(self, codeset):
        ...
        try:
            message_box(
                "Aggregate emissions",
                "Emission aggregation for codeset "
                + str(codeset)
                + " starts after clicking OK.\n",
            )
            self.task = RunBackgroundTask(
                description="Aggregate emissions",
                function=run_aggregate_emissions,
    ...
    def initUI(self):
        ...
        if type(self.stdout) == dict:
            ...
            tablewidth = (
                margins.left()
                + margins.right()
                + tableWidget.frameWidth() * 2
                + max_row_label_width
                + tableWidget.horizontalHeader().length()
                + 10
            )
            tableWidget.setFixedWidth(tablewidth * 4)
            tableWidget.setFixedHeight(
                margins.top()
                + margins.bottom()
                + tableWidget.verticalHeader().length()
                + tableWidget.horizontalHeader().width()
            )
...
class RunImportTask(QgsTask):
    ...
    def run(self):
        ...
        try:
            self.backup_path, self.proc = run_import(
                self.file_path, self.sheets, dry_run=self.dry_run
            )
            for i in range(3600):
                ...
                if self.check_process_ready():
                    ...
                    if i < 51:
                        ...
                        self.setProgress(50 + i / 1150.0 * 50.0)
    ...
    def finished(self, result):
        ...
        if result:
            ...
            stdout_files = glob.glob(
                os.path.join(gettempdir(), "cetk_import_*_stdout.log")
            )
            stderr_files = glob.glob(
                os.path.join(gettempdir(), "cetk_import_*_stderr.log")
            )
            ...
            if self.exception is None:
                ...
                if (
                    "Database unspecified does not exist, first run 'cetk create' or 'cetk migrate'"
                    in error
                ):
                    message_box(
                        "Error",
                        f"Error: a target database is not specified yet,"
    ...
    def cancel(self):
        QgsMessageLog.logMessage(
            f"Task {self.description()} was canceled", MESSAGE_CATEGORY, Qgis.Info
        )
        super().cancel()
...
class RunBackgroundTask(QgsTask):
    ...
    def run(self):
        ...
        """
        QgsMessageLog.logMessage(
            f"Started task {self.description()}", MESSAGE_CATEGORY, Qgis.Info
        )
        try:
    ...
    def finished(self, result):
        ...
        if result:
            QgsMessageLog.logMessage(
                f"Task {self.description()} completed", MESSAGE_CATEGORY, Qgis.Success
            )
            if "Rasterize emissions" in self.description():
                if self.parent.load_canvas:
                    load_rasters_to_canvas(
                        self.kwargs["outputpath"], self.parent.time_threshold
                    )
